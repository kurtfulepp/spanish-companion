...
import { ArrowLeft, ArrowRight, Camera, Check, ImagePlus, LoaderCircle, LockKeyhole, RefreshCw, Upload, X } from 'lucide-react';
...
export function PhotoVocabulary({ initialSource }: { initialSource: 'upload' | 'camera' }) {
  ...
  async function selectPhoto(blob: Blob) {
    analysisRequest.current?.abort();
    analysisRequest.current = null;
    setAnalyzing(false); setResult(null);
    const currentOperation = ++operation.current;
    stopCamera();
    setCapture('closed');
    setProcessing(true);
  ...
  async function findWords() {
    ...
    try {
      const suggestions = await requestPhotoAnalysis(photo.blob, AbortSignal.any([controller.signal, AbortSignal.timeout(45_000)]));
      if (controller.signal.aborted || operation.current !== currentOperation) return;
      if (!suggestions.items.length) {
        setError('No clear objects were found. Try a closer, well-lit photo with a few everyday things.');
      } else setResult(suggestions);
    } catch (failure) {
      if (controller.signal.aborted || operation.current !== currentOperation) return;
      setError(failure instanceof Error && !['TypeError', 'TimeoutError', 'AbortError'].includes(failure.name)
        ? failure.message : 'Photo analysis could not finish. Check your connection and try again.');
    } finally {
      if (operation.current === currentOperation) { analysisRequest.current = null; setAnalyzing(false); }
    }
  }
      ...
      <div className={styles.workspace}>
        <section className={styles.photoPanel} aria-label="Photo selection">
          <div className={styles.sourceControls} aria-label="Photo source">
            <button type="button" disabled={analyzing} aria-pressed={source === 'upload'} onClick={() => changeSource('upload')}><Upload size={17} />Upload photo</button>
            {cameraAvailable && <button type="button" disabled={analyzing} aria-pressed={source === 'camera'} onClick={() => changeSource('camera')}><Camera size={18} />Take photo</button>}
          </div>
          ...
          {capture !== 'closed' ? <div className={styles.captureArea}>
            ...
            <button className={styles.primaryButton} disabled={processing || analyzing} onClick={() => source === 'camera' ? void openCamera() : fileInput.current?.click()}>{source === 'camera' ? <><Camera size={18} />Open camera</> : <><Upload size={18} />Choose photo</>}</button>
