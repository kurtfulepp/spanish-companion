...
function cameraError(error: unknown) {
  if (error instanceof DOMException) {
    if (error.name === 'NotAllowedError' || error.name === 'SecurityError')
      return 'Camera access is blocked. Check this website’s camera permission and your device settings, or upload a photo.';
    if (error.name === 'NotFoundError')
      return 'No camera was found. You can upload a photo instead.';
    if (error.name === 'NotReadableError')
      return 'The camera could not start. Close any other app using it and try again, or upload a photo.';
  }
...
function browserConfiguration(ready: boolean) {
  ...
  const help = ios
    ...
    : android
      ? 'Allow Camera in this website’s permissions. If needed, open Android Settings → Apps → your browser → Permissions → Camera.'
      : /Windows/.test(agent)
        ? 'Allow Camera in this website’s permissions. In Windows Settings → Privacy & security → Camera, check camera access and access for desktop apps such as your browser.'
        : /Mac/.test(agent)
          ? 'Allow Camera in this website’s browser settings. If your browser is listed in System Settings → Privacy & Security → Camera, allow access there too.'
...
export function PhotoVocabulary({
  initialSource,
}: {
  initialSource: 'upload' | 'camera';
}) {
  ...
  const { cameraAvailable, nativeCapture, help } =
    browserConfiguration(hydrated);
  ...
  const onProfileChange = useCallback(
    (next: LearnerProfile) => {
      if (
        result &&
        next.proficiencyLevel &&
        result.cefr_level !== next.proficiencyLevel
      ) {
        setResult(null);
        setError(
          `Your level changed to ${next.proficiencyLevel}. Find words again to update the suggestions.`,
        );
      }
      setProfile(next);
    },
  ...
  useEffect(() => {
    ...
    void video.current.play().catch(() => {
      ...
      cancelCamera();
      setError(
        'The camera preview could not start. Try again or upload a photo.',
      );
    });
  ...
  async function openCamera() {
    ...
    if (!navigator.mediaDevices?.getUserMedia) {
      setError(
        'Camera capture is unavailable here. Open KurtES in a regular browser or upload a photo.',
      );
      return;
    }
    const currentOperation = ++operation.current;
      ...
      media.getVideoTracks().forEach((track) =>
        track.addEventListener(
          ...
          () => {
            if (operation.current !== currentOperation) return;
            cancelCamera();
            setError('The camera disconnected. Try again or upload a photo.');
          },
          { once: true },
        ),
      );
  ...
  async function capturePhoto() {
    if (!video.current?.videoWidth || !video.current.videoHeight || processing)
      return;
    const currentOperation = operation.current;
    const canvas = document.createElement('canvas');
    const scale = Math.min(
      1,
      1536 / Math.max(video.current.videoWidth, video.current.videoHeight),
    );
  ...
  async function findWords() {
    if (!photo || processing || analysisRequest.current) return;
    const currentOperation = ++operation.current;
    ...
    setError('');
    try {
      const suggestions = await requestPhotoAnalysis(
        photo.blob,
        AbortSignal.any([controller.signal, AbortSignal.timeout(45_000)]),
      );
      if (controller.signal.aborted || operation.current !== currentOperation)
        return;
      if (!suggestions.items.length) {
        setError(
          'No clear objects were found. Try a closer, well-lit photo with a few everyday things.',
        );
      ...
      setError(
        failure instanceof Error &&
          !['TypeError', 'TimeoutError', 'AbortError'].includes(failure.name)
          ? failure.message
          : 'Photo analysis could not finish. Check your connection and try again.',
      );
    } finally {
      if (operation.current === currentOperation) {
        analysisRequest.current = null;
  ...
  if (!profile.proficiencyLevel)
    return (
      <main className={styles.page}>
        <VocabularyHeader onProfileChange={onProfileChange} />
        <LevelRequired profile={profile} onProfileChange={onProfileChange} />
      </main>
    );

  const resultMatchesLevel =
    result && result.cefr_level === profile.proficiencyLevel;
  const displayedError =
    result && !resultMatchesLevel
      ? `Your level changed to ${profile.proficiencyLevel}. Find words again to update the suggestions.`
      : error;

  if (resultMatchesLevel)
    return (
      <PhotoVocabularyReview
        result={result}
      ...
      <div className={styles.content}>
        ...
        <header className={styles.heading}>
          <div>
            ...
            <h1>Photo vocabulary</h1>
            <p>Choose a photo with the things you want to learn.</p>
          </div>
          <img
        ...
        <div className={styles.workspace}>
          <section className={styles.photoPanel} aria-label="Photo selection">
            ...
            <input
              ...
              capture="environment"
              tabIndex={-1}
              aria-label="Take a photo with your device"
              onChange={(event) => {
                acceptFiles(event.currentTarget.files);
                event.currentTarget.value = '';
              }}
            ...
            {capture !== 'closed' ? (
              <div className={styles.captureArea}>
                {capture === 'live' ? (
                  ...
                  <div className={styles.waiting}>
                    ...
                    <p>
                      Your browser will ask to use the camera. You can cancel
              ...
              <div className={styles.previewArea}>
                ...
                <div className={styles.previewImage}>
                  <img src={photo.url} alt="Selected for vocabulary" />
                </div>
                <div className={styles.previewActions}>
                  ...
                  >
                    <RefreshCw size={16} />
                    {source === 'camera' ? 'Retake photo' : 'Replace photo'}
                  </button>
                  <button
              ...
              <div
                ...
                onDrop={(event) => {
                  ...
                  dragDepth.current = 0;
                  setDragging(false);
                  if (!processing) acceptFiles(event.dataTransfer.files);
                }}
              >
                <div className={styles.emptyIcon}>
                  {source === 'camera' ? (
                    <Camera size={31} strokeWidth={1.5} />
                  ) : (
                    <ImagePlus size={31} strokeWidth={1.5} />
                  )}
                </div>
                <h2>
                  {source === 'camera' ? 'Take a photo' : 'Choose your photo'}
                </h2>
                <p>
                  {source === 'camera'
                    ? 'Point your camera at a few everyday objects.'
                    : 'A room, a meal, or a few things on your desk.'}
                </p>
                <button
                  className={styles.primaryButton}
                  disabled={processing || analyzing}
                ...
                <span className={styles.fileHint}>
                  {source === 'camera'
                    ? 'Camera access starts only when you choose it.'
                    : 'Or drop one photo here · Up to 20 MB'}
                </span>
                {source === 'camera' && (
                  <button
                    className={styles.textButton}
            ...
            {photo && capture === 'closed' && (
              <div className={styles.analysisActions}>
                <p>
                  KurtES won’t save your photo. Selecting Find words sends it to
                  OpenAI for vocabulary suggestions; OpenAI may retain content
                  temporarily for safety monitoring.
                </p>
                <details>
                  <summary>Photo privacy</summary>
                  <p>
                    Your photo is kept only in memory by KurtES and released
                    when you finish or leave. Only accepted words are saved in
                    this browser.{' '}
                    <a
                ...
                {analyzing ? (
                  <>
                    <output className={styles.processing}>
                      <LoaderCircle className={styles.spinner} size={18} />
                      Finding {profile.proficiencyLevel} words…
                    </output>
                    <button
                      className={styles.secondaryButton}
                      onClick={cancelCamera}
                    ...
                    <p>
                      Cancel stops waiting; analysis already sent may still
                      incur a charge.
                    </p>
                  </>
                ) : (
                  <button
                    className={styles.primaryButton}
                    disabled={processing}
                    onClick={() => void findWords()}
                  >
                    Find {profile.proficiencyLevel} words
                    <ArrowRight size={18} />
                  </button>
                )}
              </div>
            ...
            {source === 'camera' && (
              <div className={styles.cameraHelp}>
                <button
                  className={styles.textButton}
                  aria-expanded={helpOpen}
                  aria-controls="camera-help"
                  onClick={() => setHelpOpen(!helpOpen)}
                >
                  Camera access help
                </button>
                {helpOpen && <p id="camera-help">{help}</p>}
              </div>
            )}
            <div className={styles.privacy}>
              <LockKeyhole size={16} />
              <p>Opening or replacing a photo does not send it to OpenAI.</p>
            </div>
          </section>
          <aside className={styles.guide} aria-label="Photo tips">
            <span className={styles.guideLabel}>A clear starting point</span>
            <h2>What makes a useful photo?</h2>
            <ul>
              <li>
                <span>01</span>
                <div>
                  <h3>Keep objects in view</h3>
                  <p>
                    A detailed scene can return up to 20 useful words when each
                    item is clear.
                  </p>
                </div>
              </li>
              <li>
                <span>02</span>
                <div>
                  <h3>Use good light</h3>
                  <p>A sharp, well-lit photo is easier to read.</p>
                </div>
              </li>
              <li>
                <span>03</span>
                <div>
                  <h3>Choose everyday things</h3>
                  <p>Start with the objects you see and use often.</p>
                </div>
              </li>
            </ul>
            <div className={styles.guideNote}>
              <Camera size={20} strokeWidth={1.5} />
              <p>You can replace or remove your photo at any time.</p>
            </div>
          </aside>
        </div>
      </div>
