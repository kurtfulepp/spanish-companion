...
export function LearningHeader({ active, onProfileChange = ignoreProfileChange }: { active: LearningArea; onProfileChange?: (profile: LearnerProfile) => void }) {
  ...
  return <header className="app-header h-auto min-h-16 flex-wrap gap-2 py-2"><a href="/home" aria-label="KurtES home"><Brand compact /></a><nav className="hidden items-center gap-1 md:flex" aria-label="Learning areas">{sections.map((section) => <a key={section.id} className={`nav-link ${section.id === active ? 'nav-link-active' : ''}`} href={learningPath(section.id)} aria-current={section.id === active ? 'page' : undefined}>{section.label}</a>)}</nav><div className="flex items-center gap-1 sm:gap-2"><button onClick={() => void signOut()} className="rounded-full px-2.5 py-2 text-sm font-semibold text-[var(--brand-ink)] transition hover:bg-[var(--brand-cream)] sm:px-3">Sign out</button><ProfileDialog onProfileChange={onProfileChange} fallbackAvatarSrc={DEFAULT_PROFILE_AVATAR_SRC} /></div><nav className="order-3 flex w-full items-center justify-center gap-0 border-t border-border/60 pt-2 md:hidden" aria-label="Learning areas">{sections.map((section) => <a key={section.id} className={`nav-link px-2 text-xs sm:px-3 sm:text-sm ${section.id === active ? 'nav-link-active' : ''}`} href={learningPath(section.id)} aria-current={section.id === active ? 'page' : undefined}>{section.label}</a>)}</nav></header>;
}
