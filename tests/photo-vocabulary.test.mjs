...
const { analyzePhotoRequest, MAX_PHOTO_BYTES, MAX_PHOTO_VOCABULARY_ITEMS } =
  await import(
...
const request = (body = photo, headers = {}) =>
  new Request('https://kurtes.example/api/vocabulary/analyze-photo', {
    method: 'POST',
    body,
...
const mustNotCall = async () => {
  assert.fail('Unexpected downstream call');
};
...
test('rejects unsupported formats, empty uploads, and spoofed image content', async () => {
  for (const [body, headers, expected] of [
    ...
    const response = await analyzePhotoRequest(
      request(body, headers),
      dependencies({ consumeQuota: mustNotCall, fetcher: mustNotCall }),
    );
    assert.equal(response.status, expected);
  }
});
...
test('fails closed when quota is exhausted or its database is unavailable', async () => {
  for (const [consumeQuota, expected] of [
    ...
    const response = await analyzePhotoRequest(
      request(),
      dependencies({ consumeQuota, fetcher: mustNotCall }),
    );
    assert.equal(response.status, expected);
  ...
  const response = await analyzePhotoRequest(
    ...
    dependencies({
      fetcher: async (url, options) => {
        ...
        assert.equal(body.text.format.schema.properties.items.maxItems, 20);
        assert.match(body.instructions, /12 to 20 distinct/);
...
test('empty scenes are valid and repeated English entries are collapsed', async () => {
  for (const [result, count] of [
    ...
    [
      {
        ...
        items: [
          ...vocabulary.items,
          { ...vocabulary.items[0], english: ' FRYING PAN ' },
        ],
      },
      1,
    ],
    ...
    const response = await analyzePhotoRequest(
      request(),
      dependencies({ fetcher: async () => Response.json(completed(result)) }),
    );
    assert.equal(response.status, 200);
...
test('accepts up to twenty distinct suggestions for a detailed photo', async () => {
  const items = Array.from(
    { length: MAX_PHOTO_VOCABULARY_ITEMS },
    (_, index) => ({
      english: `object ${index + 1}`,
      spanish: `el objeto ${index + 1}`,
      usage_note: null,
    }),
  );
  const response = await analyzePhotoRequest(
    ...
    dependencies({
      fetcher: async () =>
        Response.json(completed({ suggested_title: 'Detailed room', items })),
    }),
  );
  assert.equal(response.status, 200);
...
test('refusals and incomplete generations cannot be mistaken for accepted vocabulary', async () => {
  for (const [output, expected] of [
    ...
    const response = await analyzePhotoRequest(
      request(),
      dependencies({ fetcher: async () => Response.json(output) }),
    );
    assert.equal(response.status, expected);
...
test('validates provider output locally even with Structured Outputs', async () => {
  for (const output of [
    ...
    completed({ ...vocabulary, items: Array(21).fill(vocabulary.items[0]) }),
    completed({
      ...vocabulary,
      items: [{ english: 'pan', spanish: 123, usage_note: null }],
    }),
    {
      status: 'completed',
      output: [
    ...
    const response = await analyzePhotoRequest(
      ...
      dependencies({
        fetcher: async () => {
          calls++;
          return Response.json(
            { error: 'test-secret private-image-bytes' },
            { status },
          );
        },
      }),
...
test('already cancelled requests never consume quota or reach OpenAI', async () => {
  ...
  controller.abort();
  const cancelled = new Request(
    'https://kurtes.example/api/vocabulary/analyze-photo',
    {
      method: 'POST',
      body: photo,
