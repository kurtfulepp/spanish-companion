...
Feedback is personal, conversational, and concise. Open a feedback result or review with the first name from the signed-in profile, then address the person directly as “you” and “your”. Never use “the learner” or “the user” in learner-facing feedback. If no display name is available, use direct second-person language without inventing a name. Use the name once at the opening rather than repeating it in every feedback block. Personalize the rendered interface locally; do not send a profile name to an AI service solely to produce this wording.

Make corrections scan like actions. Label the correction **Fix** and the replacement **Use**. Give the exact repair first in one short sentence; do not repeat the visible answer, narrate what was typed, or add a generic statement that it was wrong. For a close spelling or accent error, add a compact **You wrote / Use** comparison and highlight only the differing characters. Use **Correct — small fix** when the answer successfully communicates the target but needs one isolated, non-meaning-changing repair. Correct responses may use **Why it works**; uncertain results use **Check** and name the ambiguity directly.
...
Every vocabulary topic opens its current learning session directly from the topic card. Do not insert a topic overview or mode-choice screen between the vocabulary map and active learning. Keep **Browse all expressions** and **Check what I already know** as secondary actions in the compact learning header. The owner approved extending the Dining Out reference flow to all published topics on 19 September 2026.

New and Needs practice expression cards lead with **Learn expression** or **Learn again**. Learning reveals the Spanish, audio, usage guidance, and supported variation; **Practice expression** then opens one assisted recall response directly, without topic totals, history, another start action, or a second near-duplicate use response. Show a short usage note as a quiet sentence directly beneath the expression; do not repeat a “How it works” heading in every card. Keep **Practice expression** as the primary next action. Present **Try a variation** as a quiet secondary disclosure beside it, with the writing field and comparison hidden until selected; the optional scaffold must not visually outweigh the expression. Because the answer was just shown, this immediate activity cannot award Known. Reserve **Check now** for a later independent response to a fresh contextual prompt; grade meaning/form and use as separate dimensions from that single response. After any result, **Return to session** restores the learning session. The header-level **Check what I already know** action remains the entry to the whole-topic assessment overview.
...
For structured topic learning, distinguish the full catalog, progression sets, and the current working session. Keep the working session to six expressions, mix a limited number of due or Needs practice items with new material, and do not render an entire 24-expression set as one exercise. Show set progression only when complete published set metadata exists. Topics that currently contain a smaller exact-level personal collection use the same learning-card and assessment flow without implying that collection is a complete curriculum; label its final state **All available expressions retained**. Supported study may reveal audio, usage guidance, and a contextual variation, but must remain visibly separate from an independent check. Describe complete delayed evidence as **All expressions retained**, not mastery or verified proficiency.

Assessments prioritize identified gaps, then unassessed expressions and due reviews. Each independent check uses one typed response to a fresh contextual prompt, with meaning/form and contextual use reported separately after submission. Browsing and recently assisted practice do not award Known. “Add 12 expressions” is an explicit learner action, not an automatic page-load cost. Label personally generated material “Expanded” and preserve its stable progress identity. Keep operational model, quota, and storage details out of the interface.
...
Use the shared scrolling learning header and compact brand hero. Grammar presents the exact active profile level while the curriculum dropdown lists all six levels, including C1 and C2. Label earlier-level review and higher-level curriculum previews explicitly. Provide a grammar-type filter and expandable module outlines. Keep available rule lessons visually distinct from outlines. A rule follows Learn → Practice → Write → Review; use labeled native inputs, keyboard-operable choices, visible feedback, and separate evidence labels. Exact-form exercises count a spelling or accent error as Needs practice when that feature is the stated target; recognition and multiple-choice tasks remain binary. Account-saved practice, ungraded writing, and visit-only sample results must remain distinguishable. Do not imply mastery, complete module coverage, or teacher approval from a short lesson.

Place the optional Grammar practice-context selector in one quiet surface below the hero. Label it **Practice context**, keep **General** first, and group eligible items as **From Vocabulary** and **Your photo lists**. Explain the consequence beside the control: context changes familiar examples and the writing word bank while the rule and scoring stay the same. A selected theme says that its expressions come from prior Vocabulary practice. A selected photo list says that only reviewed and saved words are used and the image is not used. Inside a lesson, present the context as a supporting panel after the canonical rule examples and again beside original writing; it must remain visually secondary to the rule.
...
Vocabulary assessments follow [Vocabulary assessment](vocabulary-assessment.md). Keep the assisted recall and later independent contextual check visibly distinct. Each uses one keyboard-operable text field, an explicit “I don’t know” action, retained answers on failures, and “My answer may be valid” after grading. Do not show an answer or play target audio before an independent submission.
