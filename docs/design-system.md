...
| Brand badge | `--brand-logo-fill` | Gold `#ffd76a` → `#f4bd4e`, with terracotta ES lettering |
| Strong warm ink | `--brand-ink` / `#7f302b` | Text on cream and peach; light hero controls |
| Badge ink / focus | `--brand-ink-strong` / `#7b211b` | Gold badge lettering and focus outlines |
...
Use white for large type over the coral hero. For regular-size copy and controls, prefer dark terracotta on cream/gold; do not assume small white text on the orange end has adequate contrast. Retain visible keyboard focus.
