...
## Public demo retirement verification — 2026-09-05

Removed public demo links and the simulated-analysis component branch. The real review now requires API results. Moved the synthetic kitchen image and word fixtures to `tests/fixtures`; verified the image is absent from both `public/demo` and the built client assets. Browser checks found zero demo links on Vocabulary and confirmed the old `?demo=kitchen` URL opens the ordinary uploader without FPO copy. Existing saved demo-list labels remain intentional. All 30 tests, TypeScript, focused lint, and the build passed. No paid recognition request was needed for this cleanup. Remaining work is tracked in `future-cleanup.md`.
