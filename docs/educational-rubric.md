...
The eight-question grammar sample is a formative check only, with item explanations and no CEFR recommendation or assessed-profile update. A valid CEFR placement requires a defined communicative construct and a validated assessment method. The client no longer invokes the legacy placement RPC; its database revocation is prepared but pending owner approval. Historical placement records are preserved; they are not evidence of certified proficiency. See [the product QA report](qa-2026-09-19.md) for the review scope, source checks, corrections, and remaining validation work.
