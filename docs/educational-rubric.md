...
8. **Judge the intended meaning fairly.** Provide sufficient context for a unique answer, or accept and explain legitimate alternatives. Use the shared four-state feedback rule: **Correct**, **Correct — small fix**, **Needs practice**, or **Needs another check**. A small fix preserves successful evidence only when meaning and communicative function are clear and the repair is isolated capitalization, punctuation, or a non-meaning-changing spelling or accent slip. A target error in meaning, negation, gender/agreement, verb form, required complement, preposition, collocation, or register is Needs practice. A legitimate variant is Correct; an ambiguous prompt or potentially valid reading is Needs another check. Judge the target skill rather than edit distance. State a correction as the exact repair first; do not spend feedback restating the visible answer or merely declaring it wrong.
...
**Owner-adopted, 12 September 2026; interaction refined 19 September 2026.** Vocabulary uses **Known / Needs practice**, determined from an unassisted response to a fresh contextual prompt, with meaning/form and contextual use scored separately. **Correct — small fix** counts as successful vocabulary evidence while preserving the exact repair for review. Immediate post-study recall is assisted practice and cannot award Known. **Not assessed** is the absence of a usable result, including uncertainty, assistance, or a dispute; it is not a failing score. Self-ratings remain historical and never become assessed knowledge automatically. Accepted regional variants and equivalent expressions must be recognized, and feedback must isolate the target skill. For close spelling or accent errors, show the exact repair rather than only a prose explanation.

**19 September 2026:** The owner extended the target-sensitive four-state feedback rule across learning areas. Vocabulary and Conversation use the minor-fix distinction for free responses. Grammar formation remains strict when the spelling, accent, agreement, or verb form is the skill being tested. Multiple-choice activities remain binary because there is no partially formed response to evaluate. Correct and incorrect surfaces must remain visually distinct.

**19 September 2026:** Learner-facing vocabulary status follows **New → Practiced → Known → Retained**. New means there is no saved attempt. Practiced records an assisted attempt without promoting it to knowledge evidence. Known requires an unassisted contextual check. Retained requires a later unassisted success with fresh content. **Needs practice** identifies an observed gap and **Review due** makes scheduled retrieval actionable. These labels apply to every topic and custom list; they describe the evidence actually stored and never infer mastery.
...
## QA correction record — 19 September 2026

The eight-question grammar sample is a formative check only, with item explanations and no CEFR recommendation or assessed-profile update. A valid CEFR placement requires a defined communicative construct and a validated assessment method. The client no longer invokes the legacy placement RPC; client execution was revoked in production and verified for authenticated and anonymous roles. Historical placement records are preserved; they are not evidence of certified proficiency. See [the product QA report](qa-2026-09-19.md) for the review scope, source checks, corrections, and remaining validation work.
