...
See `custom-vocabulary-qa.md` for the real photo → account save → practice → fresh-tab retrieval → archive/restore/deletion validation and bounded security review. The current checkpoint supersedes the earlier pending-practice notes. Physical second-device verification and public deployment remain pending. Following explicit approval on 2026-09-05, the npm dependency advisory scan completed with zero known vulnerabilities reported; no dependency changes were needed.
