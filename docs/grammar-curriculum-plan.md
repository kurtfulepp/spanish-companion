...
Correction feedback starts with the exact repair in one short sentence. Because the original response remains visible, do not repeat it or narrate that it was wrong before giving the useful correction. Keep a legitimate ambiguity or accepted variant explicit rather than forcing a binary error message. Apply the shared **Correct / Correct — small fix / Needs practice / Needs another check** rule to open responses, but keep the decision target-sensitive: an accent or spelling repair is material when that exact formation is the grammar objective. Multiple-choice recognition and choice items remain binary.
