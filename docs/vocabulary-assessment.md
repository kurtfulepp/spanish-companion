...
The learner-facing ladder is **New → Practiced → Known → Retained**. **Needs practice** and **Review due** are action states rather than higher levels:

- **New**: no saved assessment attempt exists.
- **Practiced**: an assisted attempt or otherwise usable non-independent attempt was saved. A failed assisted attempt is instead Needs practice.
- **Known**: the independent evidence described below succeeded and its first delayed review is not yet due.
- **Review due**: a Known or Retained expression has reached its scheduled review date.
- **Retained**: a later independent check with a different prompt succeeded after the required delay.
- **Needs practice**: an assisted or independent response exposed a specific gap.

Topic sessions and custom-list assessments derive these states from existing signed receipts; no separate confidence rating is created. New uses a neutral visual treatment, Practiced and Review due use distinct gold treatments, Known and Retained use turquoise/teal treatments, and Needs practice uses coral/red. Text and icons remain present with every color.
