      ...
      <section id="practice" className="mx-auto mt-5 grid max-w-[1360px] gap-5 lg:grid-cols-[260px_minmax(0,1fr)]">
        ...
        <article className="surface order-1 flex min-h-[620px] flex-col overflow-hidden rounded-[32px] bg-[#fffaf0] lg:order-2">
          ...
          <div className="grid flex-1 items-center gap-4 px-6 py-8 sm:px-10 lg:grid-cols-[1.05fr_.8fr] lg:px-12 lg:py-5">
            <div className="relative z-10 text-center lg:text-left"><p className="text-sm font-semibold uppercase tracking-[.13em] text-[#4c7b70]">{profile.proficiencyLevel ? `${profile.proficiencyLevel} · Everyday conversation` : 'B1–B2 · Everyday conversation'}</p><h1 className="mt-7 font-heading text-[clamp(3rem,6.3vw,6.1rem)] font-semibold leading-[.92] tracking-[-.07em] text-[#173c34]">¿Qué planes<br />tienes hoy?</h1><p className="mt-6 max-w-xl text-lg leading-relaxed text-muted-foreground">Listen and respond naturally. Add one detail that makes the answer feel like yours.</p><button onClick={() => setPlayed(!played)} className="mt-7 inline-flex items-center gap-2 rounded-full bg-white px-4 py-2.5 text-sm font-semibold text-primary shadow-sm ring-1 ring-black/5 transition hover:-translate-y-0.5 hover:shadow-md active:translate-y-0" aria-label="Hear the phrase"><Volume2 className="size-[18px]" />Hear the phrase</button></div>
            <div className="relative mx-auto flex h-[320px] w-full max-w-[390px] items-end justify-center self-end lg:h-[465px]"><div className="absolute bottom-4 size-64 rounded-full bg-[#f0dfbd]/65 blur-2xl lg:size-80" /><img src="/brand/kurtes-coach.png" alt="KurtES learning coach welcoming you to today’s lesson" className="relative z-10 max-h-full w-auto drop-shadow-[0_22px_22px_rgba(78,55,32,.16)]" /></div>
          </div>
