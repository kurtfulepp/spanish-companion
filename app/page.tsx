...
export default function Home() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-background px-3 pb-4 pt-3 text-foreground sm:px-5">
      ...
      <section id="practice" className="mx-auto mt-5 grid max-w-[1360px] items-stretch gap-5 pb-5 lg:grid-cols-[1.55fr_.85fr]">
        <div className="relative min-h-[600px] overflow-hidden rounded-[30px] bg-[#173c34] px-6 py-8 text-white shadow-[0_20px_60px_rgba(19,50,43,.18)] sm:px-10 sm:py-10 lg:min-h-[620px] lg:px-14 lg:py-12">
        ...
        <aside className="grid gap-5 sm:grid-cols-2 lg:grid-cols-1 lg:grid-rows-[.8fr_1.2fr]">
          <div className="surface rounded-[26px] p-6 sm:p-7"><div className="flex items-start justify-between"><div><p className="eyebrow">Your streak</p><div className="mt-3 flex items-end gap-2"><span className="text-5xl font-semibold tracking-[-.06em]">12</span><span className="pb-1 text-muted-foreground">days</span></div></div><span className="grid size-11 place-items-center rounded-full bg-[#fff1d2] text-[#e58a22]"><Flame className="size-5 fill-current" /></span></div><div className="mt-6 grid grid-cols-7 gap-2" aria-label="Weekly activity">{['M','T','W','T','F','S','S'].map((day, i) => <div key={`${day}-${i}`} className="text-center"><span className={`mx-auto grid size-8 place-items-center rounded-full text-xs font-bold ${i < 5 ? 'bg-primary text-white' : i === 5 ? 'ring-2 ring-inset ring-primary text-primary' : 'bg-secondary text-muted-foreground'}`}>{i < 5 ? <Check className="size-4" /> : day}</span><span className="mt-2 block text-[11px] font-medium text-muted-foreground">{day}</span></div>)}</div></div>
          <div id="vocabulary" className="surface rounded-[26px] p-6 sm:p-7"><div className="flex items-center justify-between"><div><p className="eyebrow">Recently saved</p><h2 className="mt-1 text-xl font-semibold tracking-[-.03em]">Words to revisit</h2></div><span className="grid size-9 place-items-center rounded-full bg-accent"><BookOpen className="size-[18px] text-[#52776d]" /></span></div><div className="mt-5 divide-y divide-border/70">{recentWords.map((word) => <button key={word.spanish} className="group flex w-full items-center gap-3 py-3 text-left transition hover:pl-1"><span className={`word-dot ${word.tone}`} /><span className="min-w-0 flex-1"><strong className="block text-[15px] font-semibold">{word.spanish}</strong><span className="block truncate text-xs text-muted-foreground">{word.english}</span></span><ArrowRight className="size-4 text-[#75948b] transition group-hover:translate-x-1" /></button>)}</div></div>
        </aside>
      </section>

      <section id="progress" className="mx-auto grid max-w-[1360px] gap-5 pb-8 md:grid-cols-3">{[{icon: Mic, title:'Speak', copy:'Build confidence out loud', color:'coral'},{icon: Headphones, title:'Listen', copy:'Train your ear with real phrases', color:'blue'},{icon: Target, title:'Review', copy:'Keep tricky words in reach', color:'gold'}].map(({icon:Icon,title,copy,color}) => <button key={title} className="practice-card"><span className={`practice-icon ${color}`}><Icon className="size-5" /></span><span className="text-left"><strong className="block text-base">{title}</strong><span className="text-sm text-muted-foreground">{copy}</span></span><ChevronDown className="ml-auto size-4 -rotate-90 text-muted-foreground" /></button>)}</section>
    </main>
  );
}
