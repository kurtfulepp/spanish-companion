      ...
      <section id="practice" className="mx-auto mt-5 grid max-w-[1360px] gap-5 lg:grid-cols-[260px_minmax(0,1fr)]">
        <aside className="surface order-2 rounded-[28px] p-5 lg:order-1 lg:p-6">
          <div className="flex items-start justify-between lg:block">
            <div><p className="eyebrow">Your streak</p><div className="mt-2 flex items-end gap-2"><span className="text-5xl font-semibold tracking-[-.06em]">12</span><span className="pb-1 text-sm text-muted-foreground">days</span></div></div>
          ...
          <div className="mt-6 grid grid-cols-7 gap-1.5" aria-label="Weekly activity">{['M','T','W','T','F','S','S'].map((day, i) => <div key={`${day}-${i}`} className="text-center"><span className={`mx-auto grid size-7 place-items-center rounded-full text-[11px] font-semibold ${i < 5 ? 'bg-primary text-white' : i === 5 ? 'ring-1 ring-inset ring-primary text-primary' : 'bg-secondary text-muted-foreground'}`}>{i < 5 ? <Check className="size-3.5" /> : day}</span><span className="mt-1.5 block text-[10px] font-medium text-muted-foreground">{day}</span></div>)}</div>
        ...
        <article className="surface order-1 flex min-h-[620px] flex-col overflow-hidden rounded-[32px] lg:order-2">
          ...
          <div className="flex flex-1 flex-col items-center justify-center px-6 py-12 text-center sm:px-12">
            <p className="text-sm font-semibold uppercase tracking-[.13em] text-[#4c7b70]">{profile.proficiencyLevel ? `${profile.proficiencyLevel} · Everyday conversation` : 'B1–B2 · Everyday conversation'}</p>
            <button onClick={() => setPlayed(!played)} className="mt-8 grid size-16 place-items-center rounded-full bg-[#edf5f2] text-primary transition duration-200 hover:scale-105 hover:bg-[#e1f0eb] active:scale-95" aria-label="Hear the phrase"><Volume2 className="size-6" /></button>
            <h1 className="mt-8 max-w-3xl font-heading text-[clamp(3rem,7vw,6.5rem)] font-semibold leading-[.92] tracking-[-.07em] text-[#173c34]">¿Qué planes<br />tienes hoy?</h1>
            <p className="mt-7 max-w-xl text-lg leading-relaxed text-muted-foreground">Listen and respond naturally. Add one detail that makes the answer feel like yours.</p>
          </div>
          <div className="flex flex-col items-center justify-between gap-4 border-t border-border/70 bg-[#fafafa] px-6 py-5 sm:flex-row sm:px-8"><span className="text-sm text-muted-foreground">About 5 minutes · 4 activities</span><Button onClick={() => setPlayed(!played)} className="h-12 w-full rounded-full bg-primary px-7 text-[15px] font-semibold shadow-[0_8px_22px_rgba(23,60,52,.16)] hover:bg-[#245247] sm:w-auto">{played ? <Check className="size-4" /> : <Play className="size-4 fill-current" />}{played ? 'Lesson started' : 'Start lesson'}</Button></div>
        </article>
      </section>
