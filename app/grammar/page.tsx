...
import { GRAMMAR_CONTENT, type GrammarContent } from '@/lib/level-content';
import type { CEFRLevel } from '@/lib/cefr';

export default function GrammarPage() {
  const { profile, loading, setProfile } = useLearnerProfile();
  const updateProfile = useCallback((next: LearnerProfile) => setProfile(next), [setProfile]);
  const content = profile.proficiencyLevel ? GRAMMAR_CONTENT[profile.proficiencyLevel] : null;

  if (loading) return <main className="min-h-screen bg-background px-3 pt-3 sm:px-5"><LearningHeader active="grammar" onProfileChange={updateProfile} /><div className="mx-auto mt-5 h-[560px] max-w-[1360px] animate-pulse rounded-[30px] bg-white" /></main>;
  if (!content || !profile.proficiencyLevel) return <main className="min-h-screen bg-background px-3 pb-12 pt-3 text-foreground sm:px-5"><LearningHeader active="grammar" onProfileChange={updateProfile} /><LevelRequired profile={profile} onProfileChange={updateProfile} /></main>;

  return <GrammarExperience key={profile.proficiencyLevel} level={profile.proficiencyLevel} content={content} onProfileChange={updateProfile} />;
}

function GrammarExperience({ level, content, onProfileChange }: { level: CEFRLevel; content: GrammarContent; onProfileChange: (profile: LearnerProfile) => void }) {
  ...
  return (
    <main className="min-h-screen bg-background px-3 pb-12 pt-3 text-foreground sm:px-5">
      <LearningHeader active="grammar" onProfileChange={onProfileChange} />
        ...
        <article className="surface overflow-hidden rounded-[30px]">
          <div className="border-b border-border/70 bg-[#fff4dc] px-6 py-6 sm:px-8">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div><p className="eyebrow text-[#9a6720]">{level} · {content.eyebrow}</p><h1 className="mt-2 font-heading text-[clamp(2.2rem,5vw,4rem)] font-semibold leading-none tracking-[-.055em] text-[#173c34]">{content.title}</h1><p className="mt-3 max-w-2xl text-base leading-relaxed text-[#79564c]">{content.description}</p></div>
          ...
          <div className="p-6 sm:p-8">
            ...
            </> : <div className="grid min-h-[360px] place-items-center py-6 text-center"><div className="max-w-lg"><span className="mx-auto grid size-14 place-items-center rounded-full bg-[#e9f5ef] text-[#2f755f]"><Check className="size-6" /></span><p className="eyebrow mt-5">Practice complete</p><h2 className="mt-2 text-4xl font-semibold tracking-[-.055em] text-[#173c34]">{score} of {questions.length}</h2><p className="mt-3 text-base leading-relaxed text-muted-foreground">{score === questions.length ? `You completed this ${level} practice accurately.` : 'Review the explanation behind each choice, then try once more.'}</p><Button onClick={restart} className="mt-6 h-11 rounded-full px-6 font-bold"><RotateCcw className="size-4" />Practice again</Button></div></div>}
          </div>
        </article>
        <aside className="grid content-start gap-5">
          <section className="rounded-[26px] bg-[#fff0e8] p-6 ring-1 ring-[#ea6c4d]/20 sm:p-7"><p className="eyebrow text-[#b6503f]">Why this matters</p><h2 className="mt-2 text-2xl font-semibold tracking-[-.04em] text-[#7f302b]">{content.whyTitle}</h2><p className="mt-3 text-sm leading-relaxed text-[#79564c]">{content.why}</p></section>
          <section className="surface rounded-[26px] p-6 sm:p-7"><p className="eyebrow">Coming next</p><div className="mt-3 divide-y divide-border/70">{content.upcoming.map((topic) => <div key={topic.title} className="py-4 first:pt-1"><div className="flex items-center justify-between gap-3"><strong className="text-[15px] text-[#173c34]">{topic.title}</strong><span className="rounded-full bg-secondary px-2.5 py-1 text-[11px] font-bold uppercase tracking-[.06em] text-muted-foreground">Planned</span></div><p className="mt-1 text-sm text-muted-foreground">{topic.detail}</p></div>)}</div></section>
        </aside>
      </section>
    </main>
  );
