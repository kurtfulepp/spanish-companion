...
import { CONVERSATION_CONTENT } from '@/lib/level-content';

export default function ConversationPage() {
  const { profile, loading, setProfile } = useLearnerProfile();
  const [lessonOpen, setLessonOpen] = useState(false);
  const [completedLevel, setCompletedLevel] = useState<string | null>(null);
  const updateProfile = useCallback((next: LearnerProfile) => setProfile(next), [setProfile]);
  const levelContent = profile.proficiencyLevel ? CONVERSATION_CONTENT[profile.proficiencyLevel] : null;
  ...
  useEffect(() => {
    ...
    void Promise.resolve(context.registerTool({
      ...
      execute() {
        setLessonOpen(true);
        return { status: 'opened', lesson: levelContent?.lessonTitle ?? 'Spanish conversation' };
      },
    }, { signal: lifecycle.signal })).catch(() => undefined);
    return () => lifecycle.abort();
  }, [levelContent?.lessonTitle]);

  if (loading) return <main className="min-h-screen bg-background px-3 pt-3 sm:px-5"><LearningHeader active="conversation" onProfileChange={updateProfile} /><div className="mx-auto mt-5 h-72 max-w-[1360px] animate-pulse rounded-[30px] bg-white" /></main>;
  if (!profile.proficiencyLevel) return <main className="min-h-screen bg-background px-3 pb-12 pt-3 text-foreground sm:px-5"><LearningHeader active="conversation" onProfileChange={updateProfile} /><LevelRequired profile={profile} onProfileChange={updateProfile} /></main>;
  const content = levelContent!;
  const lessonComplete = completedLevel === profile.proficiencyLevel;

  return (
    <main className="min-h-screen bg-background px-3 pb-12 pt-3 text-foreground sm:px-5">
      {lessonOpen && <DailyLessonDialog open onOpenChange={setLessonOpen} level={profile.proficiencyLevel} voice={profile.voicePreference} onComplete={() => setCompletedLevel(profile.proficiencyLevel)} />}
      ...
      <section className="brand-hero mx-auto mt-5 max-w-[1360px]">
        ...
        <div className="relative grid min-h-[280px] items-center gap-5 overflow-hidden px-6 py-7 sm:px-9 lg:grid-cols-[1fr_310px] lg:px-11">
          <div className="relative z-10 max-w-3xl">
            ...
            <h1 className="mt-4 font-heading text-[clamp(2.6rem,5vw,4.8rem)] font-semibold leading-[.94] tracking-[-.06em]">{content.prompt}</h1>
            <p className="mt-4 max-w-2xl text-base leading-relaxed text-white/88">{content.instruction}</p>
            <div className="mt-6 flex flex-wrap gap-3"><Button onClick={() => setLessonOpen(true)} className="h-12 rounded-full bg-white px-6 font-bold text-[#7f302b] hover:bg-[#fff4dc]"><Play className="size-4 fill-current" />{lessonComplete ? 'Practice again' : 'Start guided lesson'}</Button><SpeechButton text={content.prompt} voice={profile.voicePreference} label="Hear the phrase" className="h-12 bg-[#fff4dc] px-5 text-[#7f302b] hover:bg-white" /></div>
          </div>
      ...
      <section className="mx-auto mt-6 grid max-w-[1360px] gap-5 lg:grid-cols-[1.15fr_.85fr]">
        <article className="surface rounded-[26px] p-6 sm:p-8" aria-labelledby="listening-warmup-title">
          <div className="flex items-start justify-between gap-5"><div><p className="eyebrow">{profile.proficiencyLevel} listening warm-up</p><h2 id="listening-warmup-title" className="mt-2 text-3xl font-semibold tracking-[-.045em]">{content.warmupTitle}</h2></div><span className="practice-icon blue"><Headphones className="size-5" /></span></div>
          <div className="mt-6 divide-y divide-border/70">{content.warmup.map((phrase) => <div key={phrase.spanish} className="flex flex-col gap-3 py-4 first:pt-0 sm:flex-row sm:items-center sm:justify-between"><div><strong className="block text-base font-semibold text-[#173c34]">{phrase.spanish}</strong><span className="mt-1 block text-sm text-muted-foreground">{phrase.english}</span></div><SpeechButton text={phrase.spanish} voice={profile.voicePreference} /></div>)}</div>
        </article>
        ...
        <aside className="grid gap-5">
          <article className="rounded-[26px] bg-[#fff4dc] p-6 ring-1 ring-[#f4bd4e]/35 sm:p-7">
            ...
            <h2 className="mt-2 text-2xl font-semibold tracking-[-.04em] text-[#694b18]">{content.lessonTitle}</h2>
            <ul className="mt-5 space-y-3 text-[15px] text-[#79564c]">{content.lessonPoints.map((point) => <li key={point} className="flex gap-3"><Check className="mt-0.5 size-4 shrink-0 text-[#c37a1d]" />{point}</li>)}</ul>
