...
import {
  MAX_PHOTO_VOCABULARY_ITEMS,
  TARGET_PHOTO_VOCABULARY_ITEMS,
} from '@/lib/photo-vocabulary-limits';

export { MAX_PHOTO_VOCABULARY_ITEMS } from '@/lib/photo-vocabulary-limits';
...
When the photo supports it, return ${TARGET_PHOTO_VOCABULARY_ITEMS} to ${MAX_PHOTO_VOCABULARY_ITEMS} distinct, clearly visible objects, ordered by usefulness and prominence.
Use fewer items when the scene does not contain enough clear, useful objects; return an empty items array if nothing is clear.
...
const schema = {
  ...
  properties: {
    ...
    items: {
      type: 'array',
      maxItems: MAX_PHOTO_VOCABULARY_ITEMS,
      items: {
        type: 'object',
        additionalProperties: false,
        properties: {
...
async function readPhoto(request: Request): Promise<Uint8Array> {
  ...
  if (declaredLength > MAX_PHOTO_BYTES) {
    throw new PhotoError(
      413,
      'photo_too_large',
      'Use a photo smaller than 2 MB.',
    );
  }
  const reader = request.body?.getReader();
  if (!reader)
    throw new PhotoError(400, 'missing_photo', 'Choose a photo first.');
...
function parseVocabulary(response: unknown): PhotoVocabulary {
  if (
    ...
    throw new PhotoError(
      502,
      'analysis_incomplete',
      'Photo analysis did not finish. Try again.',
    );
  }
  ...
  if (
    !isRecord(result) ||
    !validText(result.suggested_title, 80) ||
    !Array.isArray(result.items) ||
    result.items.length > MAX_PHOTO_VOCABULARY_ITEMS
  ) {
    throw new PhotoError(
      502,
      'invalid_analysis',
      'Photo suggestions could not be read. Try again.',
    );
  }
  ...
  for (const item of result.items) {
    if (
      !isRecord(item) ||
      !validText(item.english, 100) ||
      !validText(item.spanish, 100) ||
  ...
  try {
    ...
    if (isCrossOriginRequest(request)) {
      throw new PhotoError(
        403,
        'cross_site_request',
        'Open photo vocabulary in the app.',
      );
    }
    ...
    if (!isCEFRLevel(level)) {
      throw new PhotoError(
        ...
        'Set your Spanish level before analyzing a photo.',
      );
    }
    if (!deps.apiKey?.trim()) {
      throw new PhotoError(
        ...
        'Photo vocabulary is not configured yet.',
      );
    }
    ...
    try {
      ...
      throw new PhotoError(
        503,
        'quota_unavailable',
        'Photo vocabulary is temporarily unavailable.',
      );
    }
    if (!allowed)
      throw new PhotoError(
        429,
        'photo_limit_reached',
        'Photo limit reached. Try again later.',
      );
    ...
    try {
            ...
            input: [
              {
                ...
                content: [
                  {
                    type: 'input_text',
                    text: `Suggest vocabulary for the visible objects at CEFR level ${level}.`,
                  },
                  {
                    type: 'input_image',
                    image_url: `data:${mime};base64,${base64(bytes)}`,
                    detail: 'auto',
                  },
                ],
              },
      ...
      throw new PhotoError(
        timedOut ? 504 : 502,
        'vision_unavailable',
        'Photo analysis is unavailable. Try again.',
      );
    }
    if (!upstream.ok) {
      ...
      const status =
        upstream.status === 429 ? 429 : upstream.status === 400 ? 422 : 502;
      throw new PhotoError(
        status,
        status === 429 ? 'vision_limit_reached' : 'vision_request_failed',
        status === 429
          ? 'Photo recognition has reached its usage or spending limit. Please try again later.'
          : status === 422
            ? 'This photo could not be analyzed. Choose another photo.'
    ...
    return json({
      ...parseVocabulary(await upstream.json()),
      cefr_level: level,
    ...
    return json(
      {
        error: 'Photo analysis is unavailable. Try again.',
