...
const fields = 'id,name,words,source,cefr_level,completed,created_at,practiced_count,confident_count';
export function listRecord(owner: string, list: CustomVocabularyList) {
  ...
  return { user_id: owner, id: list.id, name: list.name.trim(), source: list.source ?? 'demo', cefr_level: list.cefrLevel ?? null, completed: list.completed, created_at: list.createdAt,
...
export async function loadCustomLists(client: SupabaseClient, owner: string): Promise<CustomVocabularyList[]> {
  ...
  for (let start = 0; ; start += pageSize) {
    ...
    lists.push(...data.map(row => ({ id: row.id, name: row.name, words: row.words, source: row.source, cefrLevel: row.cefr_level ?? undefined, completed: row.completed, createdAt: row.created_at, practicedCount: row.practiced_count, confidentCount: row.confident_count })));
